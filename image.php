...
// get base name
$baseName = basename($_GET['image'],".".$ext);

$arrImg = explode("-",$baseName);

// get image directory
$dir = $_GET['dir'];

// get new size
$size = $arrImg[count($arrImg)-1];

//get actual image name
$imgName = str_replace("-".$size,"",$baseName);

// build absolute path
if($dir == "tmpl")
   $_GET['image'] = URI::getAbsTemplatePath()."front/images/".$imgName.".".$ext;
else
    $_GET['image'] = URI::getAbsMediaPath($dir)."/".$imgName.".".$ext;

// get new width and height
$_GET['width'] = CFG::$imageSize[$size][0];
$_GET['height'] = CFG::$imageSize[$size][1];
